...
router.get("/get-user-contract2/:id", async (req, res) => {
    const id = req.params.id.trim(); // ✅ ตัด \n ออก

  try {
    const userData = await Contract2.findById(id);
    if (!userData) {
...
const sendApprovalEmail = async (contract2) => {
  try {
    // ดึง contract_id จาก Contract2
    const contract_id = contract2.contract_id;
    ...
    const mailOptions = {
      ...
      สัญญาหมายเลข ${contract2.contract_id}
...
// API อนุมัติ
router.put("/:id/approve-contract2", async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const updated = await Contract2.findByIdAndUpdate(
        id,
        {
          status: "อนุมัติ",
...
const sendRejectionEmail = async (contract2, reason = "") => {
  try {
    // ดึง contract_id จาก Contract2
    const contract_id = contract2.contract_id;
    ...
    const mailOptions = {
      ...
      สัญญาหมายเลข ${contract2.contract_id}
      ขั้นตอนที่2 ไม่ได้รับการอนุมัติ
...
// API ไม่อนุมัติ
router.put("/:id/reject-contract2", async (req, res) => {
    try {
      const { id } = req.params;
