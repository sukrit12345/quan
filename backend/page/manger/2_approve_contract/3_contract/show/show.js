...
//เเสดงรายการ
router.get("/api/contract3-summary", async (req, res) => {
  try {
    const result = await Contract1.aggregate([
      ...
      {
        $lookup: {
          from: "contract3", // ชื่อคอลเลกชันของ contract2 ใน MongoDB
      ...
      },
      {
        $project: {
          ...
          province: 1,
          contract_id: 1,
          createdAt: "$contract3_info.createdAt",
          status: "$contract3_info.status",
          approver_id_card: "$contract3_info.approver_id_card",
          contract2_id: "$contract3_info._id",
        },
      },
      {
...
router.get("/approver-info3/:approver_id_card", async (req, res) => {
  try {
...
// ลบข้อมูลตาม id
router.delete("/contract3/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const deletedContract = await Contract3.findByIdAndDelete(id);
