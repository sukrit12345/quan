...
router.get("/get-user-contract3/:id", async (req, res) => {
    const id = req.params.id.trim(); // ✅ ตัด \n ออก

  try {
    const userData = await Contract3.findById(id);
    if (!userData) {
...
const sendApprovalEmail = async (contract3) => {
  try {
    // ดึง contract_id จาก Contract2
    const contract_id = contract3.contract_id;
    ...
    const mailOptions = {
      ...
      สัญญาหมายเลข ${contract3.contract_id}
      ขั้นตอนที่3 ได้รับการอนุมัติเรียบร้อยแล้ว
...
// API อนุมัติ
router.put("/:id/approve-contract3", async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const updated = await Contract3.findByIdAndUpdate(
        id,
        {
          status: "อนุมัติ",
...
const sendRejectionEmail = async (contract3, reason = "") => {
  try {
    // ดึง contract_id จาก Contract2
    const contract_id = contract3.contract_id;
    ...
    const mailOptions = {
      ...
      สัญญาหมายเลข ${contract3.contract_id}
      ขั้นตอนที่3 ไม่ได้รับการอนุมัติ
...
// API ไม่อนุมัติ
router.put("/:id/reject-contract3", async (req, res) => {
    try {
      const { id } = req.params;
